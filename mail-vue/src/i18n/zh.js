const zh = {
    ...
    aiAgentSave: '保存',
    aiAgentProvider: '模型服务商 (Provider)',
    aiAgentProviderWorkersAI: 'Cloudflare Workers AI (内置)',
    aiAgentProviderAIGateway: 'Cloudflare AI Gateway (网关代理)',
    aiAgentProviderCustom: '自定义 API (OpenAI 兼容直连)',
    aiAgentCfAccountId: 'Cloudflare Account ID',
    aiAgentCfAccountIdPlaceholder: '例如 32 位 Hex 字符',
    aiAgentAiGatewayId: 'AI Gateway ID',
    aiAgentAiGatewayIdPlaceholder: '例如 my-mail-gateway',
    aiAgentGatewayProvider: 'Gateway 下游 Provider',
    aiAgentGatewayProviderHelp: 'Cloudflare AI Gateway 绑定的目标服务商，可选择预设服务商或 Custom 类型',
    aiAgentGatewayCustomProvider: 'Custom Provider 名称',
    aiAgentGatewayCustomProviderPlaceholder: '例如 custom-siliconflow (需以 custom- 开头)',
    aiAgentGatewayCustomProviderHelp: '对应在 Cloudflare AI Gateway 中添加的自定义 Provider 端点，必须以 custom- 开头',
    aiAgentBaseUrl: 'API Base URL',
    aiAgentBaseUrlPlaceholder: '例如 https://api.deepseek.com/v1',
    aiAgentApiKey: 'API Key',
    aiAgentApiKeyPlaceholder: '请输入 API 密钥',
    aiAgentModel: '模型名称 (Model)',
    aiAgentModelPlaceholder: '选择或直接输入模型名称',
    aiAgentFetchModels: '拉取模型列表',
    aiAgentModelsFetched: '成功获取到 {count} 个模型',
    aiAgentFetchModelsFailed: '获取模型列表失败：',
    aiAgentTestConnection: '测试连接',
    aiAgentTesting: '正在测试...',
    aiAgentTestSuccess: '连接成功',
    aiAgentTestSuccessMsg: '模型连接正常！响应耗时: {latency}ms',
    aiAgentTestFailed: '连接测试失败',
    aiAgentWorkersAIHint: '使用 Cloudflare 边缘内置原生推理，零配置开箱即用。',
    aiAgentAIGatewayHint: '通过 Cloudflare 边缘网关代理，支持边缘缓存、调用监控、限流及多模型统一接入。',
    aiAgentCustomHint: '直接连接 OpenAI、DeepSeek、通义千问、OneAPI 等兼容 OpenAI 格式的外部 API。',
